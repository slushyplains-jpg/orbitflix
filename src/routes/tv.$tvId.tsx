...
import { Star, Clock, Calendar, Tv, ExternalLink, PlayCircle } from "lucide-react";
...
function TVPage() {
  ...
  const poster = IMG_URL(show.poster_path, "w500");
  ...
  const justWatchUrl = `https://www.justwatch.com/us/search?q=${encodeURIComponent(show.name ?? "")}`;
      ...
      <div className="relative -mt-48 px-6 pb-16 md:px-16 lg:px-24">
        <div className="flex flex-col gap-8 sm:flex-row sm:items-start">
          ...
          <div className="flex-1 pt-2">
            {show.tagline && (
              <p className="mb-2 text-xs uppercase tracking-[0.25em] text-muted-foreground">{show.tagline}</p>
            )}
            ...
            <div className="mt-4 flex flex-wrap items-center gap-5 text-sm text-muted-foreground">
              ...
              {show.number_of_seasons > 0 && (
                <span className="flex items-center gap-1.5">
                  <Tv className="h-4 w-4" />{show.number_of_seasons} Season{show.number_of_seasons > 1 ? "s" : ""}
                </span>
              )}
              {show.vote_average > 0 && (
                <span className="flex items-center gap-1.5 text-yellow-400">
                  <Star className="h-4 w-4 fill-current" />{show.vote_average.toFixed(1)}
                </span>
              )}
            </div>
            ...
            {show.overview && (
              <p className="mt-5 max-w-2xl text-base leading-relaxed text-muted-foreground">{show.overview}</p>
            )}
        ...
        {seasons.length > 0 && (
          <section className="mt-14">
            <p className="mb-5 text-xs uppercase tracking-[0.25em] text-muted-foreground">Seasons</p>
            <div className="flex flex-wrap gap-3">
              {seasons.map((s) => (
                <div key={s.id} className="flex items-center gap-2 rounded-xl border border-border bg-surface px-4 py-2.5">
                  <Tv className="h-3.5 w-3.5 text-muted-foreground" />
                  <span className="text-sm font-medium">{s.name}</span>
                  {s.episode_count > 0 && (
                    <span className="text-xs text-muted-foreground">· {s.episode_count} eps</span>
                  )}
                </div>
              ))}
            </div>
        ...
        <section className="mt-14">
          ...
          <p className="mb-6 text-sm text-muted-foreground">
            Find <span className="font-medium text-foreground">"{show.name}"</span> on your streaming platforms:
          </p>
